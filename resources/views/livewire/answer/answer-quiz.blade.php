<div class="container-quiz">
    ...
    @if ($quiz->questions)
        @foreach ($quiz->questions as $question)
            <div class="container mt-4 mb-6 bg-white dark:bg-gray-800 rounded-lg overflow-hidden transform sm:w-full">
                <div class="px-6 py-4">
                    <div class="mt-4 text-sm text-gray-600 dark:text-gray-400">
                        <div class="container mt-4">
                            <div
                                class="w-full form-control shadow-none border-none rounded-md p-2 overflow-hidden text-xl">
                                {{ $question->name }}
                            </div>
                        </div>
                        @if ($question->typequestion == 1)
                            <div class="container mt-4">
                                ...
                                <x-input-error for="answers.{{ $question->id }}" />
                            </div>
                        @elseif ($question->typequestion == 2)
                            @foreach ($question->choices as $choice)
                                <div class="flex items-center container mt-4">
                                    <input wire:model="answers.{{ $question->id }}" id="default-radio-{{$question->id}}-{{$choice->id}}" type="radio" value="{{ $choice->id }}" name="question-radio-{{ $question->id }}" class="w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                    <label for="default-radio-{{$question->id}}-{{$choice->id}}" class="ml-2 dark:text-gray-300">{{ $choice->value }}</label>
                                </div>
                            @endforeach
                        @endif
                    </div>
    ...
    <div class="container mt-4 mb-6 rounded-lg overflow-hidden transform sm:w-full">
