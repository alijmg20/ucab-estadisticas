<div wire:init='loadUsers'>
        ...
        <div class="py-8">
            <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
                <div class="inline-block w-full shadow rounded-lg overflow-hidden">
                    ...
                    @if ($users && count($users) && $users->hasPages())
                        <div class="px-6 py-3">
                            {{ $users->links() }}
                        </div>
                    @endif
    ...
    <script>
        document.addEventListener('livewire:load', function() {
            Livewire.on('userAlert', (title, message) => {
                alert(title, message)
            });
            Livewire.on('userDelete', (variable) => {
                Swal.fire({
                    title: '¿Estas seguro?',
                    ...
                    if (result.isConfirmed) {
                        Livewire.emitTo("user.user-controller", "delete", variable);
                        Swal.fire(
                            'Eliminado!',
